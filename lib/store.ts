...
export type ControlState = {
  ...
  finTs: number | null;
  // Se actualiza cada vez que el supervisor da RESET. El operador hace
  // polling de /api/control y compara este valor contra el último que
...
export async function resetSesion(code: string): Promise<void> {
  // resetTs es la señal que el operador está esperando ver cambiar en su
  // polling (/api/control) para recargar sus 4 páginas — sin esto, el
  // reset nunca se propaga a la pantalla del operador.
  const control: ControlState = { estado: 'espera', inicioTs: null, finTs: null, resetTs: Date.now() };
  if (hasRedisEnv()) {
    const r = redis();
    await r.set(k(`session:${code}:control`), JSON.stringify(control), { ex: SESSION_TTL_SECONDS });
    await r.del(k(`session:${code}:maniobras`));
